...
class ObsidianReader(BaseReader):
    """
    Extended ObsidianReader for loading data from an Obsidian Vault.
    Each Document includes metadata with file name, folder path, and extracted wikilinks.
    ...
    def load_data(self, *args: Any, **load_kwargs: Any) -> List[Document]:
        """
        Walks through the vault, loads each markdown file, and augments the resulting
        Document objects with extra metadata (file name, folder path, wikilinks, and optionally tasks).
        """
        docs: List[Document] = []
        for dirpath, dirnames, filenames in os.walk(self.input_dir):
            ...
            for filename in filenames:
                if filename.endswith(".md"):
                    ...
                    # Load the markdown file. MarkdownReader may return multiple documents if it splits on headers.
                    md_docs = MarkdownReader().load_data(Path(filepath))
                    for i, doc in enumerate(md_docs):
                        # Add file-level metadata.
                        doc.metadata["file_name"] = Path(filepath).name
                        doc.metadata["folder_path"] = str(Path(filepath).parent)
                        doc.metadata["folder_name"] = str(
                            Path(filepath).parent.relative_to(self.input_dir)
                        )
                        # Extract wikilinks and add them to metadata.
                        doc.metadata["wikilinks"] = self._extract_wikilinks(doc.text)
                        ...
                        if self.extract_tasks:
                            ...
                            if self.remove_tasks_from_text:
                                # Replace the document with a new instance that has the cleaned text.
                                md_docs[i] = Document(
