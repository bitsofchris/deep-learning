...
class ResultsManager:
    ...
    def log_experiment_result(self, experiment_config, results):
        ...
        self.logger.info(
            f"Logged experiment {experiment_config.get('experiment_id')} "
            f"run {experiment_config.get('run_id')}: "
            f"accuracy={results.get('final_accuracy', 0):.2f}%"
        )

    def log_training_epoch(
        self, experiment_id, run_id, epoch, train_metrics, val_metrics, lr
    ):
    ...
    def save_final_model(
        self, model, optimizer, final_metrics, experiment_id, run_id, experiment_config
    ):
    ...
    def plot_training_curves(self, experiment_ids=None, save=True):
        ...
        grouped = (
            df.groupby(["experiment_id", "epoch"])
            .agg(
                {
        ...
        for exp_id in grouped["experiment_id"].unique():
            ...
            ax2.plot(
                exp_data["epoch"],
                exp_data["val_accuracy_mean"],
                label=f"Exp {exp_id} Val",
                alpha=0.8,
            )
            ax2.fill_between(
                exp_data["epoch"],
    ...
    def generate_experiment_report(self):
        ...
        summary = (
            results_df.groupby(["data_ordering", "curriculum_strategy"])
            .agg(
                {
