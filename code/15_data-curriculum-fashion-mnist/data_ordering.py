...
class DataOrdering:
    ...
    def get_fashion_mnist_datasets(self, data_dir: str = "data") -> Tuple[Dataset, Dataset]:
        ...
        self.logger.info(f"Loaded Fashion-MNIST: {len(train_dataset)} train, {len(test_dataset)} test")
    ...
    def get_curriculum_ordering(self,
                               images: torch.Tensor,
                               labels: torch.Tensor, 
                               strategy: str = "medoid_first",
                               n_clusters: int = 50,
                               pca_components: int = 50,
                               use_pca: bool = True) -> Tuple[np.ndarray, Dict]:
    ...
    def create_data_loaders(self,
                           ...
                           data_dir: str = "data") -> Tuple[DataLoader, DataLoader, Optional[Dict]]:
        ...
        if ordering_strategy == "random":
            ...
            self.logger.info(f"Extracted {len(all_images)} training samples for clustering")
            ...
            self.logger.info(f"Created curriculum ordering data loader with {len(ordered_dataset)} samples")
    ...
    def create_curriculum_phases(self,
                                train_dataset: Dataset,
                                ordered_indices: np.ndarray,
                                phase_sizes: List[int]) -> List[DataLoader]:
        ...
        for i, size in enumerate(phase_sizes):
            ...
            self.logger.info(f"Created curriculum phase {i+1} with {len(phase_dataset)} samples")
            
        return phase_loaders


def get_data_loaders(ordering_strategy: str = "random",
                    curriculum_params: Optional[Dict] = None,
                    batch_size: int = 64,
                    num_workers: int = 2,
                    data_dir: str = "data",
                    random_state: int = 42) -> Tuple[DataLoader, DataLoader, Optional[Dict]]:
