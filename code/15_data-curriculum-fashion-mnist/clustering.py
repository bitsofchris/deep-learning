...
class FAISSClustering:
    ...
    def _prepare_features(self, images: torch.Tensor) -> np.ndarray:
        ...
        if self.use_pca:
            self.logger.info(
                f"Applying PCA to reduce dimensionality to {self.pca_components}"
            )
            self.pca = PCA(
                n_components=self.pca_components, random_state=self.random_state
            )
    ...
    def fit_predict(self, images: torch.Tensor) -> np.ndarray:
        ...
        if n_samples > max_samples:
            self.logger.info(
                f"Subsampling {max_samples} from {n_samples} samples for clustering"
            )
        ...
        self.logger.info(
            f"Feature matrix shape for clustering: {features_for_clustering.shape}"
        )
        ...
        self.logger.info(
            f"Clustering completed. Found {len(np.unique(labels))} clusters"
        )
    ...
    def _analyze_clusters(self, features: np.ndarray, labels: np.ndarray) -> None:
        ...
        self.logger.info(
            f"Quartile sizes - Q1: {len(self.quartile_indices['q1'])}, "
    ...
    def get_curriculum_ordering(self, strategy: str = "medoid_first") -> np.ndarray:
        ...
        if strategy == "medoid_first":
            ...
            ordered_indices = np.random.RandomState(self.random_state).permutation(
                n_samples
            )
        ...
        self.logger.info(
            f"Generated curriculum ordering with strategy '{strategy}': {len(ordered_indices)} samples"
        )
    ...
    def _create_round_robin_ordering(self) -> np.ndarray:
        ...
        self.logger.info(
            f"Round-robin: Using {len(cluster_samples)} non-empty clusters"
        )
        ...
        self.logger.info(
            f"Round-robin ordering: {len(ordered_indices)} samples across {max_cluster_size} rounds"
        )
    ...
    def get_cluster_info(self) -> Dict:
        ...
        return {
            ...
            "pca_variance_explained": (
                self.pca.explained_variance_ratio_.sum() if self.pca else None
            ),
        }


def cluster_fashion_mnist(
    images: torch.Tensor,
    labels: torch.Tensor,
    n_clusters: int = 50,
    pca_components: int = 50,
