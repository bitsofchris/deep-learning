...
class CurriculumExperimentRunner:
    ...
    def run_single_experiment(self, 
                             ...
                             experiment_id: int, 
                             run_id: int,
                             curriculum_data: Dict = None) -> Dict:
        ...
        if config["ordering_method"] == "random":
            ...
            ordered_dataset = OrderedDataset(
                curriculum_data["train_dataset"], 
                curriculum_data["ordered_indices"]
            )
        ...
        self.logger.info(f"Loaded data: {len(train_loader.dataset)} train, {len(test_loader.dataset)} test")
        ...
        if cluster_info and config["ordering_method"] == "curriculum":
            ...
            if curriculum_data is not None:
                ...
                if viz_images:
                    ...
                    viz_cluster_info['medoid_indices'] = list(range(min(10, len(medoid_indices))))
                    viz_cluster_info['edge_indices'] = list(range(min(10, len(medoid_indices)), 
        ...
        self.logger.info(f"Experiment {experiment_id}, run {run_id} completed: "
        ...
        for exp_idx, config in enumerate(all_experiments):
            ...
            if config["ordering_method"] == "curriculum":
                ...
                if cache_key not in curriculum_cache:
                    self.logger.info("Computing curriculum ordering (will be reused across runs)...")
                    ...
                    self.logger.info(f"Reusing cached curriculum ordering for key: {cache_key}")
            ...
            for run in range(1, config["runs"] + 1):
                ...
                results = self.run_single_experiment(config, experiment_id, run, curriculum_data)
    ...
    def _generate_experiment_summary(self, results_df: pd.DataFrame):
        ...
        if 'random' in results_df['ordering_method'].values and 'curriculum' in results_df['ordering_method'].values:
            random_acc = results_df[results_df['ordering_method'] == 'random']['final_accuracy'].mean()
            curriculum_acc = results_df[results_df['ordering_method'] == 'curriculum']['final_accuracy'].mean()
