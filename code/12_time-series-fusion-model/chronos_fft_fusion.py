...
class FFTFeatureExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        top_k_indices = (
            torch.topk(magnitude[:, 1 : len(x[0]) // 2], k, dim=-1).indices + 1
        )
...
class ChronosFFTFusion(nn.Module):
    ...
    def forward(self, x: torch.Tensor, horizon: int = 1) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            chronos_features = chronos_outputs.last_hidden_state.mean(
                dim=1
            )  # Pool over sequence
...
def evaluate_models(
    ...
    improvement = (
        (results["vanilla_mse"] - results["fft_mse"]) / results["vanilla_mse"] * 100
    )
...
if __name__ == "__main__":
    ...
    train_losses, val_losses = train_model(
        model, X_train, y_train, X_val, y_val, epochs=10
    )
